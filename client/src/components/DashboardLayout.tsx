    ...
    <div style={brandStyle} className="min-h-screen bg-[#171A17] text-[#F1F0EA]">
      ...
      <div className={cn("min-h-screen transition-[margin] duration-200", collapsed ? "lg:ml-[72px]" : "lg:ml-[268px]")}>
        <header className="sticky top-0 z-20 flex h-[72px] items-center gap-3 border-b border-[#42463F] bg-[#171A17]/95 px-4 backdrop-blur lg:px-8">
          ...
          {activeInstitution && <div className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/[.04] px-2.5 py-1.5 sm:flex"><span className="flex h-6 w-6 items-center justify-center overflow-hidden rounded-md" style={{ backgroundColor: primaryColor }}>{activeInstitution.logoUrl ? <img src={activeInstitution.logoUrl} alt="" className="h-full w-full object-contain p-0.5" /> : <Landmark className="h-3.5 w-3.5 text-[#171A17]" />}</span><span className="max-w-[170px] truncate text-xs font-semibold text-[#E5E8E1]">{activeInstitution.name}</span></div>}
          <InstitutionSwitcher />
          <NotificationCenter />
