...
import { canManageCommercial, hasValidExamChoiceOptions } from "./permissions";
...
function validateExamQuestionSet(questions: Array<{ questionType: string; options?: unknown; correctAnswer?: unknown }>) {
  for (const question of questions) {
    if ((question.questionType === "single_choice" || question.questionType === "multiple_choice") && !hasValidExamChoiceOptions(question.options, question.correctAnswer)) throw new TRPCError({ code: "BAD_REQUEST", message: "Las preguntas de selección deben tener cuatro opciones A, B, C y D, una correcta y tres distractores." });
  }
}
...
export const appRouter = router({
  ...
  wellbeing: router({
    surveys: protectedProcedure.input(z.object({ audience: z.enum(["students", "teachers", "alumni"]).optional() }).optional()).query(({ ctx, input }) => db.listWellbeingSurveys(ctx.institutionId ?? 1, input?.audience)),
    create: protectedProcedure.input(z.object({ title: z.string().trim().min(2).max(220), audience: z.enum(["students", "teachers", "alumni"]), triggerType: z.enum(["period_end", "annual_alumni", "manual"]), anonymous: z.boolean(), questions: z.array(z.object({ prompt: z.string().trim().min(2).max(500), questionType: z.enum(["scale", "single_choice", "text"]), options: z.array(z.string().trim().min(1).max(120)).nullable().optional(), sortOrder: z.number().int().min(0) })).min(1).max(50) })).mutation(async ({ ctx, input }) => { assertInstitutionAdmin(ctx.user); return db.createWellbeingSurvey({ institutionId: ctx.institutionId ?? 1, title: input.title, audience: input.audience, triggerType: input.triggerType, anonymous: input.anonymous, createdBy: ctx.user.id }, input.questions); }),
    submit: protectedProcedure.input(z.object({ surveyId: z.number().int().positive(), answers: z.array(z.object({ questionId: z.number().int().positive(), answer: z.string().max(5000) })).min(1).max(50) })).mutation(async ({ ctx, input }) => { const result = await db.submitWellbeingResponse({ institutionId: ctx.institutionId ?? 1, surveyId: input.surveyId, respondentUserId: ctx.user.id }, input.answers); if (!result) throw new TRPCError({ code: "BAD_REQUEST", message: "La encuesta o las respuestas no son válidas." }); return result; }),
  }),
  alumni: router({
    list: protectedProcedure.query(async ({ ctx }) => { assertInstitutionAdmin(ctx.user); return db.listAlumniProfiles(ctx.institutionId ?? 1); }),
    upsert: protectedProcedure.input(z.object({ studentId: z.number().int().positive(), graduationYear: z.number().int().min(1900).max(2200).nullable().optional(), currentCompany: z.string().trim().max(180).nullable().optional(), jobTitle: z.string().trim().max(180).nullable().optional(), employmentStatus: z.enum(["employed", "self_employed", "seeking", "studying", "unknown"]), consentToContact: z.boolean(), notes: z.string().trim().max(5000).nullable().optional() })).mutation(async ({ ctx, input }) => { assertInstitutionAdmin(ctx.user); const result = await db.upsertAlumniProfile({ ...input, institutionId: ctx.institutionId ?? 1 }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "El estudiante no pertenece a la institución activa." }); return result; }),
    addInteraction: protectedProcedure.input(z.object({ alumniProfileId: z.number().int().positive(), interactionType: z.enum(["call", "email", "event", "survey", "note"]), summary: z.string().trim().min(2).max(5000) })).mutation(async ({ ctx, input }) => { assertInstitutionAdmin(ctx.user); const result = await db.addAlumniInteraction({ ...input, institutionId: ctx.institutionId ?? 1, createdBy: ctx.user.id }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Perfil de egresado no encontrado." }); return result; }),
  }),
  studyPlans: router({
    list: protectedProcedure.query(({ ctx }) => db.listStudyPlans(ctx.institutionId ?? 1)),
    get: protectedProcedure.input(z.object({ planId: z.number().int().positive() })).query(async ({ ctx, input }) => { const result = await db.getStudyPlanById(input.planId, ctx.institutionId ?? 1); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Plan de estudio no encontrado." }); return result; }),
    create: protectedProcedure.input(z.object({ name: z.string().trim().min(2).max(180), version: z.string().trim().min(1).max(40), description: z.string().trim().max(5000).nullable().optional(), subjects: z.array(z.object({ subjectId: z.number().int().positive(), semester: z.number().int().min(1).max(20), credits: z.number().int().min(0).max(100), required: z.boolean() })).min(1).max(200) })).mutation(async ({ ctx, input }) => { assertInstitutionAdmin(ctx.user); const result = await db.createStudyPlanWithSubjects({ institutionId: ctx.institutionId ?? 1, name: input.name, version: input.version, description: input.description, createdBy: ctx.user.id }, input.subjects); if (!result) throw new TRPCError({ code: "BAD_REQUEST", message: "Una o más materias no pertenecen a la institución activa." }); return result; }),
  }),
  exams: router({
    ...
    createManual: protectedProcedure.input(z.object({ subjectId: z.number().int().positive(), moduleId: z.number().int().positive().nullable().optional(), title: z.string().trim().min(2).max(220), description: z.string().trim().max(5000).nullable().optional(), instructions: z.string().trim().max(10000).nullable().optional(), durationMinutes: z.number().int().min(1).max(600), maxAttempts: z.number().int().min(1).max(10), requiresCamera: z.boolean(), requiresMicrophone: z.boolean(), status: z.enum(["draft", "published"]), questions: z.array(z.object({ questionType: z.enum(["open", "single_choice", "multiple_choice", "true_false", "fill_blank", "matching", "ordering"]), prompt: z.string().trim().min(2).max(10000), options: z.unknown().optional(), correctAnswer: z.unknown().optional(), rubric: z.string().trim().max(5000).nullable().optional(), points: z.number().int().min(1).max(100), sortOrder: z.number().int().min(0) })).min(1).max(100) })).mutation(async ({ ctx, input }) => { await assertExamManager(ctx.user, input.subjectId, ctx.institutionId ?? 1); validateExamQuestionSet(input.questions); const result = await db.createExamWithQuestions({ ...input, institutionId: ctx.institutionId ?? 1, createdBy: ctx.user.id }, input.questions); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "La materia no pertenece a la institución activa." }); return result; }),
    generateWithAI: protectedProcedure.input(z.object({ subjectId: z.number().int().positive(), topic: z.string().trim().min(2).max(500), questionCount: z.number().int().min(1).max(30), difficulty: z.enum(["basic", "intermediate", "advanced"]), questionTypes: z.array(z.enum(["open", "single_choice", "multiple_choice", "true_false", "fill_blank", "matching", "ordering"])).min(1).max(7) })).mutation(async ({ ctx, input }) => { await assertExamManager(ctx.user, input.subjectId, ctx.institutionId ?? 1); const response = await invokeLLM({ messages: [{ role: "system", content: "Genera exámenes educativos rigurosos. Devuelve únicamente JSON válido y crea preguntas claras, variadas y calificables." }, { role: "user", content: `Materia: ${input.subjectId}. Tema: ${input.topic}. Dificultad: ${input.difficulty}. Cantidad: ${input.questionCount}. Tipos permitidos: ${input.questionTypes.join(", ")}. Las preguntas abiertas deben incluir rúbrica y no deben tener respuesta correcta automática. Para single_choice y multiple_choice debes devolver exactamente cuatro opciones rotulables A, B, C y D, tres distractores y una respuesta correcta; correctAnswer debe coincidir con una opción o con un arreglo de opciones.` }], response_format: { type: "json_schema", json_schema: { name: "exam_proposal", strict: true, schema: { type: "object", properties: { title: { type: "string" }, instructions: { type: "string" }, questions: { type: "array", items: { type: "object", properties: { questionType: { type: "string", enum: input.questionTypes }, prompt: { type: "string" }, options: { type: ["array", "null"], items: { type: "string" }, minItems: 4, maxItems: 4 }, correctAnswer: {}, rubric: { type: ["string", "null"] }, points: { type: "integer" } }, required: ["questionType", "prompt", "options", "correctAnswer", "rubric", "points"], additionalProperties: false } } }, required: ["title", "instructions", "questions"], additionalProperties: false } } } }); const content = response.choices?.[0]?.message?.content; if (typeof content !== "string") throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "La IA no devolvió una propuesta válida." }); try { const proposal = JSON.parse(content); validateExamQuestionSet(proposal.questions ?? []); return proposal; } catch (error) { if (error instanceof TRPCError) throw error; throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "La propuesta de IA no tiene un formato válido." }); } }),
    ...
    attemptReview: protectedProcedure.input(z.object({ attemptId: z.number().int().positive() })).query(async ({ ctx, input }) => { if (ctx.user.role !== "admin" && ctx.user.role !== "teacher") throw new TRPCError({ code: "FORBIDDEN", message: "Solo un tutor puede revisar intentos." }); const candidate = await db.getExamAttemptForTeacher(input.attemptId, ctx.institutionId ?? 1); if (!candidate?.exam) return null; await assertExamManager(ctx.user, candidate.exam.subjectId, ctx.institutionId ?? 1); const result = candidate; if (!result) return result; return { ...result, recordingUrl: result.attempt.recordingKey ? await storageGetSignedUrl(result.attempt.recordingKey) : null }; }),
    gradeAttempt: protectedProcedure.input(z.object({ attemptId: z.number().int().positive(), manualScores: z.array(z.object({ answerId: z.number().int().positive(), score: z.number().int().min(0).max(100), feedback: z.string().trim().max(5000).nullable().optional() })), tutorFeedback: z.string().trim().max(5000).nullable().optional() })).mutation(async ({ ctx, input }) => { if (ctx.user.role !== "admin" && ctx.user.role !== "teacher") throw new TRPCError({ code: "FORBIDDEN", message: "Solo un tutor puede calificar intentos." }); const candidate = await db.getExamAttemptForTeacher(input.attemptId, ctx.institutionId ?? 1); if (!candidate?.exam) throw new TRPCError({ code: "NOT_FOUND", message: "Intento no disponible." }); await assertExamManager(ctx.user, candidate.exam.subjectId, ctx.institutionId ?? 1); const result = await db.gradeExamAttempt({ ...input, institutionId: ctx.institutionId ?? 1 }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Intento no disponible." }); const exam = await db.getExamWithQuestions(result.examId, ctx.institutionId ?? 1); if (exam) await db.createGrade({ studentId: result.studentId, subjectId: exam.exam.subjectId, period: new Date().getFullYear().toString(), title: exam.exam.title, score: result.finalScore, maxScore: result.maxScore, notes: input.tutorFeedback ?? null, gradedBy: ctx.user.id, institutionId: ctx.institutionId ?? 1 }); return result; }),
    ...
    submitAttempt: protectedProcedure.input(z.object({ attemptId: z.number().int().positive(), answers: z.array(z.object({ questionId: z.number().int().positive(), answer: z.unknown() })).max(100) })).mutation(async ({ ctx, input }) => { if (ctx.user.role !== "student") throw new TRPCError({ code: "FORBIDDEN", message: "Solo estudiantes pueden enviar exámenes." }); const result = await db.submitExamAttempt({ ...input, institutionId: ctx.institutionId ?? 1, studentId: ctx.user.id }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Intento no disponible." }); if (!result.requiresTutorReview) { const exam = await db.getExamWithQuestions(result.examId, ctx.institutionId ?? 1); if (exam) await db.createGrade({ studentId: result.studentId, subjectId: exam.exam.subjectId, period: new Date().getFullYear().toString(), title: exam.exam.title, score: result.autoScore, maxScore: result.maxScore, notes: "Calificación automática del examen objetivo.", gradedBy: ctx.user.id, institutionId: ctx.institutionId ?? 1 }); } return result; }),
    ...
    uploadRecording: protectedProcedure.input(z.object({ attemptId: z.number().int().positive(), dataBase64: z.string().min(1000).max(80_000_000), mimeType: z.enum(["video/webm", "video/mp4", "audio/webm", "audio/mp4"]) })).mutation(async ({ ctx, input }) => { if (ctx.user.role !== "student") throw new TRPCError({ code: "FORBIDDEN", message: "Solo el estudiante puede subir la grabación de su intento." }); const uploaded = await storagePut(`exams/recordings/${ctx.institutionId ?? 1}/${ctx.user.id}/${input.attemptId}`, Buffer.from(input.dataBase64, "base64"), input.mimeType); const result = await db.updateExamAttemptRecording({ attemptId: input.attemptId, institutionId: ctx.institutionId ?? 1, studentId: ctx.user.id, recordingKey: uploaded.key, recordingMimeType: input.mimeType, recordingConsentAt: new Date() }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Intento no disponible." }); return result; }),
  }),
  financeAccess: router({
    status: protectedProcedure.query(async ({ ctx }) => { if (ctx.user.role !== "student") return { status: "current" as const, balanceCents: 0, blocked: false }; return db.getFinancialAccessStatus(ctx.institutionId ?? 1, ctx.user.id); }),
  }),
  lms: router({
    ...
    gradeTask: protectedProcedure.input(z.object({ contentId: z.number().int().positive(), studentId: z.number().int().positive(), score: z.number().int().min(0).max(1000), maxScore: z.number().int().min(1).max(1000), feedback: z.string().trim().max(5000).nullable().optional() })).mutation(async ({ ctx, input }) => { if (ctx.user.role !== "admin" && ctx.user.role !== "teacher") throw new TRPCError({ code: "FORBIDDEN", message: "Solo un tutor puede calificar tareas LMS." }); const result = await db.gradeLmsTask({ ...input, institutionId: ctx.institutionId ?? 1, gradedBy: ctx.user.id }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "La tarea o el estudiante no pertenecen a la institución activa." }); return result; }),
