...
export const appRouter = router({
  ...
  exams: router({
    ...
    attemptReview: protectedProcedure.input(z.object({ attemptId: z.number().int().positive() })).query(async ({ ctx, input }) => { if (ctx.user.role !== "admin" && ctx.user.role !== "teacher") throw new TRPCError({ code: "FORBIDDEN", message: "Solo un tutor puede revisar intentos." }); const result = await db.getExamAttemptForTeacher(input.attemptId, ctx.institutionId ?? 1); if (!result) return result; return { ...result, recordingUrl: result.attempt.recordingKey ? await storageGetSignedUrl(result.attempt.recordingKey) : null }; }),
