...
export async function createInstitution(input: {
  ...
  const result = await db.insert(institutions).values({
    ...
    primaryColor: input.primaryColor || "#B69A5E",
    secondaryColor: input.secondaryColor || "#1B201D",
    status: "active",
  });
...
export async function deleteInstitution(institutionId: number) {
  if (institutionId === 1)
    throw new Error("La institución principal no se puede eliminar.");
  const db = await requireDb();
  return db.transaction(async tx => {
    const administrators = await tx
      .select({ userId: institutionMemberships.userId, email: users.email })
    ...
    await tx
      .delete(moduleAssessmentAttempts)
      .where(eq(moduleAssessmentAttempts.institutionId, institutionId));
    await tx
      .delete(lessonProgress)
      .where(eq(lessonProgress.institutionId, institutionId));
    await tx
      .delete(submissions)
      .where(eq(submissions.institutionId, institutionId));
    ...
    await tx
      ...
      .where(eq(notificationPreferences.institutionId, institutionId));
    await tx
      .delete(enrollments)
      .where(eq(enrollments.institutionId, institutionId));
    await tx.delete(grades).where(eq(grades.institutionId, institutionId));
    await tx
      .delete(liveClasses)
      .where(eq(liveClasses.institutionId, institutionId));
    await tx
      .delete(activities)
      .where(eq(activities.institutionId, institutionId));
    await tx
      .delete(moduleAssessments)
      .where(eq(moduleAssessments.institutionId, institutionId));
    await tx
      .delete(courseLessons)
      .where(eq(courseLessons.institutionId, institutionId));
    await tx
      .delete(courseModules)
      .where(eq(courseModules.institutionId, institutionId));
    await tx
      .delete(courseResources)
      .where(eq(courseResources.institutionId, institutionId));
    await tx
      .delete(competencies)
      .where(eq(competencies.institutionId, institutionId));
    ...
    for (const administrator of administrators) {
      if (
        !administrator.email ||
        administrator.email.toLowerCase() === "wilinton@gmail.com"
      )
        continue;
      const remaining = await tx
        .select({ id: institutionMemberships.id })
        .from(institutionMemberships)
        .where(eq(institutionMemberships.userId, administrator.userId))
        .limit(1);
      if (!remaining.length) {
...
export async function listInstitutionsForUser(userId: number) {
  ...
  return db
    .select({
      ...
      primaryColor: institutions.primaryColor,
      secondaryColor: institutions.secondaryColor,
      role: institutionMemberships.role,
    })
    .from(institutionMemberships)
    .innerJoin(
      institutions,
      eq(institutions.id, institutionMemberships.institutionId)
    )
    .where(
      and(
...
export async function getInstitutionForUser(
  ...
  if (requestedInstitutionId)
    conditions.push(
      eq(institutionMemberships.institutionId, requestedInstitutionId)
    );
  const [membership] = await db
    .select({ institutionId: institutionMemberships.institutionId })
    .from(institutionMemberships)
    .innerJoin(
      institutions,
      eq(institutions.id, institutionMemberships.institutionId)
    ...
    .orderBy(asc(institutionMemberships.institutionId))
    .limit(1);
...
export async function getUserByIdInInstitution(
  userId: number,
  institutionId = 1
) {
  const db = await requireDb();
...
export async function createLocalUser(input: {
  ...
  await db.insert(institutionMemberships).values({
    institutionId,
    userId,
    role: input.role === "admin" ? "admin" : (input.role ?? "student"),
  });
...
export async function createRecoveryRequest(input: {
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(recoveryRequests.institutionId, input.institutionId),
        eq(recoveryRequests.userId, input.userId),
  ...
  const [created] = await db
    .select()
    .from(recoveryRequests)
    .where(eq(recoveryRequests.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
export async function listRecoveryRequests(institutionId: number) {
  ...
  return db
    .select({
      ...
      institutionId: recoveryRequests.institutionId,
      userId: recoveryRequests.userId,
      status: recoveryRequests.status,
      requestedAt: recoveryRequests.requestedAt,
      resolvedAt: recoveryRequests.resolvedAt,
      resolvedBy: recoveryRequests.resolvedBy,
      name: users.name,
      email: users.email,
      role: users.role,
    })
    ...
    .where(eq(recoveryRequests.institutionId, institutionId))
    .orderBy(desc(recoveryRequests.requestedAt));
}
export async function resolveRecoveryRequest(input: {
  requestId: number;
  institutionId: number;
  resolvedBy: number;
}) {
  ...
  await db
    ...
    .where(
      and(
        eq(recoveryRequests.id, input.requestId),
        eq(recoveryRequests.institutionId, input.institutionId),
        eq(recoveryRequests.status, "pending")
      )
    );
...
export async function setTemporaryPassword(input: {
  ...
  const db = await requireDb();
  const member = await getUserByIdInInstitution(
    input.userId,
    input.institutionId
  );
...
export async function deleteUserAccount(userId: number) {
  ...
  const createdAssessments = await db
    .select({ id: moduleAssessments.id })
    .from(moduleAssessments)
    .where(eq(moduleAssessments.createdBy, userId));
  const assessmentIds = createdAssessments.map(assessment => assessment.id);
  if (activityIds.length)
    await db
      .delete(submissions)
      .where(inArray(submissions.activityId, activityIds));
  if (assessmentIds.length)
    await db
      .delete(moduleAssessmentAttempts)
      .where(inArray(moduleAssessmentAttempts.assessmentId, assessmentIds));
  if (studentIds.length) {
    await db
      .delete(moduleAssessmentAttempts)
      .where(inArray(moduleAssessmentAttempts.studentId, studentIds));
    await db
      .delete(lessonProgress)
      .where(inArray(lessonProgress.studentId, studentIds));
    await db
      .delete(enrollments)
  ...
  if (assessmentIds.length)
    await db
      .delete(moduleAssessments)
      .where(inArray(moduleAssessments.id, assessmentIds));
...
export async function getStudentById(studentId: number, institutionId = 1) {
  ...
  const result = await db
    ...
    .from(students)
    .where(
      and(eq(students.id, studentId), eq(students.institutionId, institutionId))
    )
    .limit(1);
  return result[0];
}
...
export async function getStudentForUser(
  ...
  const result = await db
    .select()
    .from(students)
    .where(and(condition, eq(students.institutionId, institutionId)))
    .limit(1);
  return result[0];
}
...
export async function listSubjectsForUser(user: {
  id: number;
  role: string;
  email?: string | null;
  institutionId?: number;
...
export async function getSubjectById(subjectId: number, institutionId = 1) {
  ...
  const result = await db
    ...
    .from(subjects)
    .where(
      and(eq(subjects.id, subjectId), eq(subjects.institutionId, institutionId))
    )
    .limit(1);
  return result[0];
}
...
export async function createSubjectWithCurriculum(input: {
  ...
  for (const studentId of input.studentIds ?? [])
    await enrollStudent(studentId, subjectId, institutionId);
  ...
  for (const competency of input.competencies ?? [])
    await createCompetency({ ...competency, subjectId, institutionId });
  for (
    let moduleIndex = 0;
    moduleIndex < (input.modules ?? []).length;
    moduleIndex += 1
  ) {
    ...
    for (
      let lessonIndex = 0;
      lessonIndex < courseModule.lessons.length;
      lessonIndex += 1
    ) {
    ...
    if (courseModule.assessment)
      await createModuleAssessment({
...
export async function updateSubject(
  ...
  await db
    .update(subjects)
    .set({ ...input, active: input.active ? 1 : 0 })
    .where(
      and(eq(subjects.id, subjectId), eq(subjects.institutionId, institutionId))
    );
}
...
export async function deleteSubject(subjectId: number, institutionId = 1) {
  ...
  const activityRows = await db
    ...
    .where(
      and(
        ...
        eq(activities.institutionId, institutionId)
      )
    );
  ...
  const moduleRows = await db
    ...
    .where(
      and(
        eq(courseModules.subjectId, subjectId),
  ...
  const lessonRows = moduleIds.length
    ? await db
        ...
        .where(
          and(
            inArray(courseLessons.moduleId, moduleIds),
            eq(courseLessons.institutionId, institutionId)
          )
        )
    : [];
  const lessonIds = lessonRows.map(row => row.id);
  const assessmentRows = moduleIds.length
    ? await db
        .select({ id: moduleAssessments.id })
        .from(moduleAssessments)
        .where(
          and(
            inArray(moduleAssessments.moduleId, moduleIds),
            eq(moduleAssessments.institutionId, institutionId)
          )
        )
    : [];
  ...
  if (assessmentIds.length)
    await db
      ...
      .where(
        and(
          inArray(moduleAssessmentAttempts.assessmentId, assessmentIds),
          eq(moduleAssessmentAttempts.institutionId, institutionId)
        )
      );
  if (lessonIds.length)
    await db
      ...
      .where(
        and(
          ...
          eq(lessonProgress.institutionId, institutionId)
        )
      );
  if (moduleIds.length)
    await db
  ...
  await db
    ...
    .where(
      and(
        eq(courseResources.subjectId, subjectId),
        eq(courseResources.institutionId, institutionId)
      )
    );
  await db
    .delete(competencies)
    .where(
      and(
        ...
        eq(competencies.institutionId, institutionId)
      )
    );
  await db
    .delete(enrollments)
  ...
  await db
    ...
    .where(
      and(
        ...
        eq(grades.institutionId, institutionId)
      )
    );
  await db
    .delete(subjects)
...
export async function enrollStudent(
  ...
  await db
    .insert(enrollments)
    .values({ studentId, subjectId, institutionId })
    .onDuplicateKeyUpdate({ set: { studentId, institutionId } });
}
...
export async function listEnrollmentsForSubject(
  subjectId: number,
  institutionId = 1
) {
  const db = await requireDb();
...
export async function createCourseResource(input: {
  ...
  const result = await db
    ...
    .values({ ...input, institutionId: input.institutionId ?? 1 });
  return Number(result[0].insertId);
}
...
export async function listCompetencies(subjectId: number, institutionId = 1) {
  ...
  return db
    ...
    .orderBy(competencies.createdAt);
}
...
export async function listCourseModulesForSubject(
  subjectId: number,
  institutionId = 1
) {
  const db = await requireDb();
  ...
  return modules.map(courseModule => ({
    ...
    lessons: lessons
      ...
      .map(lesson => ({
        ...lesson,
        keyTopics: readStringArray(lesson.keyTopics),
      })),
  }));
}
...
export async function createModuleAssessment(input: {
  moduleId: number;
  title: string;
  description: string;
  questions: AssessmentQuestion[];
  ...
  const db = await requireDb();
  const result = await db.insert(moduleAssessments).values({
    ...input,
    institutionId: input.institutionId ?? 1,
  ...
  if (input.status === "published") {
    const [module] = await db
      .select({ subjectId: courseModules.subjectId })
      .from(courseModules)
...
export async function listModuleAssessments(
  ...
  const assessments = await db
    ...
    .where(
      and(
        eq(moduleAssessments.moduleId, moduleId),
        eq(moduleAssessments.institutionId, institutionId)
      )
    )
    .orderBy(desc(moduleAssessments.createdAt));
  return assessments.map(assessment => ({
    ...assessment,
    questions: readAssessmentQuestions(assessment.questions).map(question =>
      includeAnswers
        ? question
        : {
            id: question.id,
...
export async function getModuleAssessmentById(
  assessmentId: number,
  institutionId = 1
) {
  const db = await requireDb();
  const assessment = (
    await db
      ...
      .where(
        and(
          eq(moduleAssessments.id, assessmentId),
          eq(moduleAssessments.institutionId, institutionId)
        )
      )
      .limit(1)
...
export async function createModuleAssessmentAttempt(input: {
  assessmentId: number;
  studentId: number;
  ...
  const db = await requireDb();
  const result = await db.insert(moduleAssessmentAttempts).values({
    ...input,
...
export async function listAssessmentAttemptsForStudent(
  studentId: number,
  ...
  return db
    .select()
    .from(moduleAssessmentAttempts)
    .where(
      and(
        eq(moduleAssessmentAttempts.studentId, studentId),
        inArray(moduleAssessmentAttempts.assessmentId, assessmentIds)
      )
    )
    .orderBy(desc(moduleAssessmentAttempts.completedAt));
}
...
export async function setLessonCompletion(input: {
  ...
  const db = await requireDb();
  const lesson = await getCourseLessonById(
    input.lessonId,
    input.institutionId ?? 1
  );
  if (!lesson) return;
...
export async function listCompletedLessonsForStudent(
  studentId: number,
  subjectId: number,
  institutionId = 1
) {
  ...
  return db
    ...
    .where(
      and(
        ...
        eq(messages.institutionId, institutionId),
...
export async function createMessage(input: {
  ...
  await notifyUserIfEnabled(
    input.recipientId,
    "message",
    "Nuevo mensaje académico",
    "Has recibido un mensaje de tu docente o estudiante.",
    `/materias/${input.subjectId}`,
    input.institutionId ?? 1
  );
  return Number(result[0].insertId);
}

export async function listActivitiesForUser(user: {
  id: number;
  role: string;
  email?: string | null;
  institutionId?: number;
...
export async function updateActivity(
  ...
  const [existing] = await db
    .select({
      ...
      title: activities.title,
      status: activities.status,
      institutionId: activities.institutionId,
    })
    .from(activities)
...
export async function listGradesForUser(user: {
  id: number;
  role: string;
  email?: string | null;
  institutionId?: number;
  ...
  if (user.role === "admin")
    return query.where(tenantCondition).orderBy(desc(grades.gradedAt));
  if (user.role === "teacher")
    return query
      .where(and(tenantCondition, eq(subjects.teacherId, user.id)))
      .orderBy(desc(grades.gradedAt));
  const student = await getStudentForUser(user.id, user.email, institutionId);
  if (!student) return [];
  return query
    .where(and(tenantCondition, eq(grades.studentId, student.id)))
    .orderBy(desc(grades.gradedAt));
}
...
export async function getGradeById(gradeId: number, institutionId = 1) {
  ...
  const result = await db
    .select()
    .from(grades)
    .where(and(eq(grades.id, gradeId), eq(grades.institutionId, institutionId)))
    .limit(1);
  return result[0];
}
...
export async function listSubmissionsForUser(user: {
  id: number;
  role: string;
  email?: string | null;
  institutionId?: number;
  ...
  if (!student) return [];
  return query
    .where(and(tenantCondition, eq(submissions.studentId, student.id)))
    .orderBy(desc(submissions.submittedAt));
}
...
export async function gradeSubmission(
  submissionId: number,
  input: { score: number; feedback?: string | null; gradedBy: number }
) {
  const db = await requireDb();
  await db
    .update(submissions)
...
export async function listLiveClassesForUser(user: {
  id: number;
  role: string;
  email?: string | null;
  institutionId?: number;
  ...
  const query = db
    .select(base)
    .from(liveClasses)
    .innerJoin(subjects, eq(subjects.id, liveClasses.subjectId));
  if (user.role === "admin")
    return query
      .where(
        and(
          eq(liveClasses.institutionId, institutionId),
          eq(subjects.institutionId, institutionId)
        )
      )
  ...
  return query
    ...
    .where(
      and(
        eq(enrollments.studentId, student.id),
        eq(liveClasses.status, "published"),
        eq(liveClasses.institutionId, institutionId),
        eq(subjects.institutionId, institutionId),
        eq(enrollments.institutionId, institutionId)
      )
...
export async function getLiveClassById(id: number, institutionId = 1) {
  ...
  const result = await db
    ...
    .from(liveClasses)
    .where(
      and(eq(liveClasses.id, id), eq(liveClasses.institutionId, institutionId))
    )
    .limit(1);
  return result[0];
}
...
export async function updateLiveClass(
  ...
  const [existing] = await db
    .select({
      ...
      title: liveClasses.title,
      status: liveClasses.status,
      institutionId: liveClasses.institutionId,
    })
    .from(liveClasses)
...
export async function createNotification(input: {
  userId: number;
  institutionId?: number;
  type: "academic" | "assessment" | "live_class" | "message" | "system";
  title: string;
  message: string;
  href?: string | null;
}) {
...
async function notifyUserIfEnabled(
  userId: number,
  type: "academic" | "assessment" | "live_class" | "message",
  title: string,
  message: string,
  href: string,
  institutionId = 1
  ...
  const enabled =
    type === "academic"
      ? preferences.academicEnabled
      : type === "assessment"
        ? preferences.assessmentEnabled
        : type === "live_class"
          ? preferences.liveClassEnabled
...
async function notifySubjectStudents(
  subjectId: number,
  type: "academic" | "assessment" | "live_class",
  title: string,
  message: string,
  href: string,
  institutionId = 1
  ...
  await Promise.all(
    recipients
      .filter(recipient => recipient.role === "student")
      .map(recipient =>
        notifyUserIfEnabled(
          recipient.id,
          type,
...
export async function updateNotificationPreferences(
  userId: number,
  input: {
    academicEnabled: number;
    assessmentEnabled: number;
    liveClassEnabled: number;
  ...
  await db
    .insert(notificationPreferences)
    .values({ userId, institutionId: input.institutionId ?? 1, ...input })
...
export async function getDashboardStats(user: {
  ...
  if (user.role === "admin") {
    ...
    const [subjectTotal] = await db
      .select({ value: count() })
      .from(subjects)
      .where(
        and(eq(subjects.active, 1), eq(subjects.institutionId, institutionId))
      );
    ...
    const [average] = await db
      .select({ value: avg(grades.score) })
      .from(grades)
      .where(eq(grades.institutionId, institutionId));
    return {
      students: Number(studentTotal?.value ?? 0),
      subjects: Number(subjectTotal?.value ?? 0),
      activities: Number(activityTotal?.value ?? 0),
      average: Number(average?.value ?? 0),
    };
  }
  if (user.role === "teacher") {
    const [subjectTotal] = await db
    ...
    const [studentTotal] = await db
      .select({ value: count(sql`distinct ${enrollments.studentId}`) })
      .from(enrollments)
      .innerJoin(subjects, eq(subjects.id, enrollments.subjectId))
      .where(
        and(
  ...
  const [activityTotal] = await db
    .select({ value: count() })
    .from(activities)
    .innerJoin(enrollments, eq(enrollments.subjectId, activities.subjectId))
    .where(
      and(
        eq(enrollments.studentId, student.id),
        eq(enrollments.institutionId, institutionId),
...
export async function listProspects(
  institutionId: number,
  status?: "new" | "contacted" | "interested" | "admitted" | "enrolled" | "lost"
) {
  const db = await requireDb();
...
export async function getProspectById(id: number, institutionId: number) {
  ...
  const [prospect] = await db
    ...
    .from(prospects)
    .where(
      and(eq(prospects.id, id), eq(prospects.institutionId, institutionId))
    )
    .limit(1);
  return prospect;
}
...
export async function createProspect(input: {
  ...
  const [created] = await db
    .select()
    .from(prospects)
    .where(eq(prospects.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
  ...
  await db.transaction(async tx => {
    ...
    if (input.status && input.status !== current.status) {
      await tx.insert(prospectActivities).values({
        ...
        summary: `Estado cambiado de ${current.status} a ${input.status}`,
      });
    }
  });
...
export async function addProspectActivity(input: {
  ...
  const [created] = await db
    ...
    .where(eq(prospectActivities.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
  ...
  return db
    ...
    .where(
      and(
        ...
        eq(prospectActivities.institutionId, institutionId)
      )
    )
    .orderBy(desc(prospectActivities.createdAt));
}

export async function convertProspectToStudent(input: {
  prospectId: number;
  institutionId: number;
  actorId: number;
}) {
  ...
  const [existing] = await db
    ...
    .where(
      and(
        eq(students.institutionId, input.institutionId),
        eq(students.email, prospect.email)
      )
    )
    .limit(1);
  if (existing) {
    await updateProspect(
      input.prospectId,
      input.institutionId,
      { status: "enrolled", convertedStudentId: existing.id } as never,
      input.actorId
    );
    return existing;
  }
  const result = await db.insert(students).values({
    ...
    documentId: prospect.documentId,
  ...
  await updateProspect(
    input.prospectId,
    input.institutionId,
    { status: "enrolled", convertedStudentId: studentId } as never,
    input.actorId
  );
  return db
    .select()
...
export async function createAdmissionApplication(input: {
  ...
  if (
    input.prospectId &&
    !(await getProspectById(input.prospectId, input.institutionId))
  )
    return undefined;
  const result = await db
    .insert(admissionApplications)
    .values({ ...input, status: "submitted", submittedAt: new Date() });
  const [created] = await db
    .select()
    .from(admissionApplications)
    .where(eq(admissionApplications.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function updateAdmissionApplication(input: {
  ...
  await db
    ...
    .where(
      and(
        eq(admissionApplications.id, input.id),
        eq(admissionApplications.institutionId, input.institutionId)
      )
    );
  ...
  return db
    ...
    .where(
      and(
        ...
        eq(admissionDocuments.institutionId, institutionId)
      )
    )
    .orderBy(desc(admissionDocuments.uploadedAt));
}

export async function addAdmissionDocument(input: {
  institutionId: number;
  applicationId: number;
  documentType: string;
  originalName: string;
  ...
  const db = await requireDb();
  if (
    !(await getAdmissionApplication(input.applicationId, input.institutionId))
  )
    return undefined;
  const result = await db
  ...
  const [created] = await db
    .select()
    .from(admissionDocuments)
    .where(eq(admissionDocuments.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function reviewAdmissionDocument(input: {
  ...
  await db
    ...
    .where(
      and(
        eq(admissionDocuments.id, input.id),
        eq(admissionDocuments.institutionId, input.institutionId)
      )
    );
...
export async function createPreEnrollmentRequest(input: {
  institutionId: number;
  applicationId: number;
  period: string;
}) {
  ...
  if (
    ...
    return undefined;
  const result = await db
    .insert(preEnrollmentRequests)
    .values({ ...input, status: "submitted" });
  const [created] = await db
    .select()
    .from(preEnrollmentRequests)
    .where(eq(preEnrollmentRequests.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function listPreEnrollmentRequests(institutionId: number) {
  ...
  return db
    .select()
    .from(preEnrollmentRequests)
    .where(eq(preEnrollmentRequests.institutionId, institutionId))
    .orderBy(desc(preEnrollmentRequests.requestedAt));
}

export async function listMyAdmissionApplications(
  institutionId: number,
  ...
  return db
    ...
    .where(
      and(
        eq(admissionApplications.institutionId, institutionId),
        eq(admissionApplications.applicantUserId, applicantUserId)
      )
    )
    .orderBy(desc(admissionApplications.createdAt));
}
...
export async function bulkCreateProspects(
  ...
  return db
    .select()
    .from(prospects)
    .where(eq(prospects.institutionId, institutionId))
    .orderBy(desc(prospects.createdAt));
}
...
export async function listClassSchedules(institutionId: number) {
  ...
  return db
    ...
    .innerJoin(
      subjects,
      and(
        eq(subjects.id, classSchedules.subjectId),
        eq(subjects.institutionId, institutionId)
      )
    )
...
export async function createClassSchedule(input: {
  ...
  const [subject] = await db
    ...
    .where(
      and(
        ...
        eq(subjects.institutionId, input.institutionId)
      )
    )
    .limit(1);
  ...
  const [created] = await db
    .select()
    .from(classSchedules)
    .where(eq(classSchedules.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function listStudyPlans(institutionId: number) {
  ...
  return db
    .select()
    .from(studyPlans)
    .where(eq(studyPlans.institutionId, institutionId))
...
export async function createStudyPlan(input: {
  ...
  const [created] = await db
    .select()
    .from(studyPlans)
    .where(eq(studyPlans.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function issueCertificate(input: {
  ...
  const [created] = await db
    .select()
    .from(certificates)
    .where(eq(certificates.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function verifyCertificate(token: string) {
  ...
  const [certificate] = await db
    ...
    .where(
      and(
        eq(certificates.verificationToken, token),
...
export async function listFinancialCharges(institutionId: number) {
  const db = await requireDb();
  return db
    .select({ charge: financialCharges, studentName: students.fullName })
    .from(financialCharges)
    .innerJoin(
      students,
      and(
        eq(students.id, financialCharges.studentId),
        eq(students.institutionId, institutionId)
      )
    )
    .where(eq(financialCharges.institutionId, institutionId))
    .orderBy(desc(financialCharges.createdAt));
}

export async function createFinancialCharge(input: {
  institutionId: number;
  studentId: number;
  concept: string;
  amountCents: number;
  ...
  const [created] = await db
    .select()
    .from(financialCharges)
    .where(eq(financialCharges.id, Number(result[0].insertId)))
    .limit(1);
  await db
...
export async function listFinancialPayments(institutionId: number) {
  const db = await requireDb();
  return db
    .select({ payment: financialPayments, studentName: students.fullName })
    .from(financialPayments)
    .innerJoin(
      students,
      and(
        eq(students.id, financialPayments.studentId),
        eq(students.institutionId, institutionId)
      )
    )
    .where(eq(financialPayments.institutionId, institutionId))
    .orderBy(desc(financialPayments.paidAt));
}

export async function createFinancialPayment(input: {
  institutionId: number;
  studentId: number;
  ...
  await db
    ...
    .set({
      balanceCents: sql`GREATEST(0, ${financialAccounts.balanceCents} - ${input.amountCents})`,
      status: "current",
    })
    .where(
      and(
        eq(financialAccounts.institutionId, input.institutionId),
  ...
  if (input.chargeId)
    await db
      ...
      .where(
        and(
          eq(financialCharges.id, input.chargeId),
          eq(financialCharges.institutionId, input.institutionId)
        )
      );
  const [created] = await db
    .select()
    .from(financialPayments)
    .where(eq(financialPayments.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}

export async function createFinancialExpense(input: {
  institutionId: number;
  concept: string;
  amountCents: number;
  category: string;
  ...
  const [created] = await db
    ...
    .where(eq(financialExpenses.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}
...
export async function listFinancialExpenses(institutionId: number) {
  ...
  return db
    .select()
    .from(financialExpenses)
    .where(eq(financialExpenses.institutionId, institutionId))
    .orderBy(desc(financialExpenses.incurredAt));
}

export async function listLmsContents(
  institutionId: number,
  subjectId?: number,
  studentId?: number
) {
  if (
    studentId &&
    (await getFinancialAccessStatus(institutionId, studentId)).blocked
  )
    return [];
  const db = await requireDb();
  const rows = await db
    .select({ content: lmsContents, subjectName: subjects.name })
    .from(lmsContents)
    .innerJoin(
      subjects,
      and(
        eq(subjects.id, lmsContents.subjectId),
        eq(subjects.institutionId, institutionId)
      )
    )
    .where(
      and(
        eq(lmsContents.institutionId, institutionId),
        ...(subjectId ? [eq(lmsContents.subjectId, subjectId)] : []),
        ...(studentId ? [eq(lmsContents.published, 1)] : [])
      )
    )
    .orderBy(asc(lmsContents.sortOrder), asc(lmsContents.createdAt));
  if (!studentId) return rows;
  const progress = await db
    .select()
    .from(lmsContentProgress)
    .where(
      and(
        eq(lmsContentProgress.institutionId, institutionId),
        eq(lmsContentProgress.studentId, studentId)
      )
    );
  const completed = new Map(
    progress.map(item => [item.contentId, item.completedAt])
  );
  return rows.map(row => ({
    ...row,
    completedAt: completed.get(row.content.id) ?? null,
  }));
}

export async function createLmsContent(input: {
  institutionId: number;
  subjectId: number;
  moduleId?: number | null;
  title: string;
  contentType: "reading" | "video" | "link" | "file" | "task";
  body?: string | null;
  url?: string | null;
  fileKey?: string | null;
  createdBy: number;
  ...
  const result = await db
    .insert(lmsContents)
    .values({ ...input, published: 1, sortOrder: 0 });
  const [created] = await db
    .select()
    .from(lmsContents)
    .where(eq(lmsContents.id, Number(result[0].insertId)))
    .limit(1);
  return created;
}

export async function markLmsContentComplete(input: {
  institutionId: number;
  contentId: number;
  studentId: number;
  completed: boolean;
  ...
  const [content] = await db
    ...
    .where(
      and(
        eq(lmsContents.id, input.contentId),
        eq(lmsContents.institutionId, input.institutionId),
        eq(lmsContents.published, 1)
      )
    )
    .limit(1);
  if (!content) return undefined;
  await db
    .insert(lmsContentProgress)
    .values({
      institutionId: input.institutionId,
...
export function validateExamAvailabilityWindow(
  ...
  if (availableFrom && availableUntil && availableUntil <= availableFrom) {
    throw new Error(
      "La fecha de cierre debe ser posterior a la fecha de activación."
    );
  }
}
...
export async function createExamWithQuestions(
  ...
  const [subject] = await db
    ...
    .limit(1);
  if (!subject) return undefined;
  validateExamAvailabilityWindow(input.availableFrom, input.availableUntil);
  const result = await db.insert(exams).values({
    ...input,
    requiresCamera: input.requiresCamera ? 1 : 0,
    requiresMicrophone: input.requiresMicrophone ? 1 : 0,
...
export async function deleteExam(examId: number, institutionId: number) {
  const db = await requireDb();
  return db.transaction(async tx => {
    const [exam] = await tx
      .select({ id: exams.id })
      .from(exams)
      .where(and(eq(exams.id, examId), eq(exams.institutionId, institutionId)))
      .limit(1);
    ...
    const attempts = await tx
      ...
      .where(
        and(
          ...
          eq(examAttempts.institutionId, institutionId)
        )
      );
    const attemptIds = attempts.map(item => item.id);
    if (attemptIds.length) {
      await tx
        .delete(examProctoringEvents)
        .where(
          and(
            eq(examProctoringEvents.institutionId, institutionId),
            inArray(examProctoringEvents.attemptId, attemptIds)
          )
        );
      await tx
        .delete(examAnswers)
        .where(
          and(
            eq(examAnswers.institutionId, institutionId),
            inArray(examAnswers.attemptId, attemptIds)
          )
        );
    }
    await tx
    ...
    await tx
      ...
      .where(
        and(
          ...
          eq(examQuestions.institutionId, institutionId)
        )
      );
    await tx
      .delete(exams)
...
export async function listExams(
  ...
  const db = await requireDb();
  return db
    .select({ exam: exams, subjectName: subjects.name })
    .from(exams)
    .innerJoin(
      subjects,
      and(
    ...
    .where(
      and(
        ...
        ...(subjectId ? [eq(exams.subjectId, subjectId)] : []),
        ...(studentId ? [eq(exams.status, "published")] : [])
      )
    )
...
export async function startExamAttempt(input: {
  ...
  if (exam.availableFrom && now < exam.availableFrom)
    throw new Error(
      `El examen estará disponible el ${exam.availableFrom.toLocaleString()}.`
    );
  if (exam.availableUntil && now > exam.availableUntil)
    throw new Error("La fecha límite para presentar este examen ya pasó.");
  if (
    (await getFinancialAccessStatus(input.institutionId, input.studentId))
      .blocked
  )
    throw new Error(
      "Tu acceso académico está bloqueado por mora. Comunícate con la institución."
    );
  if (
    (exam.requiresCamera && !input.cameraGranted) ||
    (exam.requiresMicrophone && !input.microphoneGranted)
  )
    throw new Error("Se requieren cámara y micrófono para iniciar el examen.");
  const attempts = await db
    ...
    .where(
      and(
        ...
        eq(examAttempts.studentId, input.studentId)
      )
    );
  if (attempts.length >= exam.maxAttempts)
    throw new Error("Se alcanzó el número máximo de intentos permitido.");
  ...
  const result = await db.insert(examAttempts).values({
    ...
    studentId: input.studentId,
    attemptNumber,
    cameraGranted: input.cameraGranted ? 1 : 0,
    microphoneGranted: input.microphoneGranted ? 1 : 0,
    status: "in_progress",
  });
  ...
  await db.insert(examProctoringEvents).values([
    {
      institutionId: input.institutionId,
      attemptId,
      eventType: "camera_granted",
    },
    {
      institutionId: input.institutionId,
      attemptId,
      eventType: "microphone_granted",
    },
  ]);
  return {
    attemptId,
...
export async function submitExamAttempt(input: {
  ...
  const questions = await db
    ...
    .where(
      and(
        ...
        eq(examQuestions.institutionId, input.institutionId)
      )
    );
  let autoScore = 0;
  const values = input.answers.map(answer => {
    ...
    return {
      ...
      autoScore: objective ? score : null,
      isReviewed: objective ? 1 : 0,
    };
  });
  ...
  await db
    .update(examAttempts)
    .set({
      status: hasOpen ? "under_review" : "graded",
      submittedAt: new Date(),
      autoScore,
      finalScore: hasOpen ? null : autoScore,
    })
  ...
  return {
    ...
    autoScore,
    maxScore,
    requiresTutorReview: hasOpen,
  };
}

export async function addExamProctoringEvent(input: {
  attemptId: number;
  institutionId: number;
  studentId: number;
  eventType:
...
export async function listExamAttemptsForTeacher(
  examId: number,
  institutionId: number
) {
  const db = await requireDb();
  return db
    .select({ attempt: examAttempts, studentName: students.fullName })
    .from(examAttempts)
    .leftJoin(
      students,
      and(
        eq(students.id, examAttempts.studentId),
        eq(students.institutionId, institutionId)
      )
    )
    .where(
...
export async function getExamAttemptForTeacher(
  attemptId: number,
  institutionId: number
) {
  const db = await requireDb();
  ...
  const [exam] = await db
    ...
    .from(exams)
    .where(
      and(eq(exams.id, attempt.examId), eq(exams.institutionId, institutionId))
    )
    .limit(1);
...
export async function gradeExamAttempt(input: {
  ...
  const [attempt] = await db
    ...
    .limit(1);
  if (!attempt) return undefined;
  for (const item of input.manualScores)
    await db
      .update(examAnswers)
      .set({
      ...
      .where(
        and(
          eq(examAnswers.id, item.answerId),
          eq(examAnswers.attemptId, input.attemptId),
          eq(examAnswers.institutionId, input.institutionId)
        )
      );
  const total =
    input.manualScores.reduce((sum, item) => sum + item.score, 0) +
    (attempt.autoScore ?? 0);
  const questions = await db
  ...
  await db
    .update(examAttempts)
    .set({
      manualScore: input.manualScores.reduce(
        (sum, item) => sum + item.score,
        0
      ),
      finalScore: total,
      tutorFeedback: input.tutorFeedback ?? null,
      status: "graded",
    })
    .where(eq(examAttempts.id, input.attemptId));
  return {
...
export async function createStudyPlanWithSubjects(
  input: {
    institutionId: number;
    name: string;
    version: string;
  ...
  const db = await requireDb();
  const validSubjects = entries.length
    ? await db
  ...
  const [plan] = await db
    ...
    .where(
      and(
        ...
        eq(studyPlans.institutionId, institutionId)
      )
    )
    .limit(1);
  if (!plan) return undefined;
  const subjectsInPlan = await db
    .select({ relation: studyPlanSubjects, subject: subjects })
    .from(studyPlanSubjects)
    .innerJoin(
      subjects,
      and(
        eq(subjects.id, studyPlanSubjects.subjectId),
        eq(subjects.institutionId, institutionId)
      )
    )
    .where(
      and(
        eq(studyPlanSubjects.studyPlanId, planId),
        eq(studyPlanSubjects.institutionId, institutionId)
      )
    )
    .orderBy(asc(studyPlanSubjects.semester), asc(subjects.name));
  const subjectIds = subjectsInPlan.map(item => item.subject.id);
  const competenciesInPlan = subjectIds.length
    ? await db
        .select()
        .from(competencies)
        .where(
          and(
            eq(competencies.institutionId, institutionId),
            inArray(competencies.subjectId, subjectIds)
          )
        )
    : [];
...
export async function refreshFinancialAccountStatus(
  institutionId: number,
  studentId: number
) {
  const db = await requireDb();
  const [account] = await db
    ...
    .where(
      and(
        eq(financialAccounts.institutionId, institutionId),
        eq(financialAccounts.studentId, studentId)
      )
    )
    .limit(1);
  if (!account)
    return { status: "current" as const, balanceCents: 0, blocked: false };
  const overdue =
    account.balanceCents > 0 &&
    !!account.dueAt &&
    account.dueAt.getTime() < Date.now();
  ...
  if (status !== account.status)
    await db
      ...
      .set({
        status,
        blockedAt:
          status === "blocked" ? (account.blockedAt ?? new Date()) : null,
      })
      .where(
        and(
...
export async function getFinancialAccessStatus(
  institutionId: number,
  studentId: number
) {
...
export async function gradeLmsTask(input: {
  institutionId: number;
  contentId: number;
  studentId: number;
  score: number;
  ...
  const [content] = await db
    ...
    .where(
      and(
        eq(lmsContents.id, input.contentId),
        eq(lmsContents.institutionId, input.institutionId),
        eq(lmsContents.contentType, "task")
      )
    )
    .limit(1);
  if (!content) return undefined;
...
export async function listWellbeingSurveys(
  institutionId: number,
  audience?: "students" | "teachers" | "alumni"
) {
  const db = await requireDb();
  return db
    .select()
    ...
    .where(
      and(
        eq(wellbeingSurveys.institutionId, institutionId),
        eq(wellbeingSurveys.active, 1),
        ...(audience ? [eq(wellbeingSurveys.audience, audience)] : [])
      )
    )
    .orderBy(desc(wellbeingSurveys.createdAt));
}
...
export async function createWellbeingSurvey(
  input: {
    ...
    title: string;
    audience: "students" | "teachers" | "alumni";
    triggerType: "period_end" | "annual_alumni" | "manual";
    anonymous: boolean;
    createdBy: number;
  },
  questions: Array<{
    prompt: string;
    questionType: "scale" | "single_choice" | "text";
    options?: string[] | null;
    sortOrder: number;
  }>
) {
  ...
  const result = await db
    ...
    .values({ ...input, anonymous: input.anonymous ? 1 : 0, active: 1 });
  const surveyId = Number(result[0].insertId);
  if (questions.length)
    await db.insert(wellbeingSurveyQuestions).values(
      questions.map(question => ({
        ...question,
        institutionId: input.institutionId,
        surveyId,
  ...
  return {
    ...
    survey: (
      await db
        .select()
        .from(wellbeingSurveys)
        .where(eq(wellbeingSurveys.id, surveyId))
        .limit(1)
    )[0],
  };
}

export async function submitWellbeingResponse(
  input: {
    institutionId: number;
    surveyId: number;
  ...
  const [survey] = await db
    ...
    .where(
      and(
        eq(wellbeingSurveys.id, input.surveyId),
        eq(wellbeingSurveys.institutionId, input.institutionId),
        eq(wellbeingSurveys.active, 1)
      )
    )
    .limit(1);
  ...
  const questions = await db
    .select()
    .from(wellbeingSurveyQuestions)
    .where(
      and(
        eq(wellbeingSurveyQuestions.surveyId, input.surveyId),
        eq(wellbeingSurveyQuestions.institutionId, input.institutionId)
      )
    );
  if (
    answers.some(
    ...
    return undefined;
  const response = await db.insert(wellbeingSurveyResponses).values({
    ...input,
    respondentUserId: survey.anonymous
      ? null
      : (input.respondentUserId ?? null),
  });
  const responseId = Number(response[0].insertId);
  if (answers.length)
    await db.insert(wellbeingSurveyAnswers).values(
      answers.map(answer => ({
        ...answer,
        responseId,
        institutionId: input.institutionId,
...
export async function listAlumniProfiles(institutionId: number) {
  ...
  return db
    ...
    .innerJoin(
      students,
      and(
        eq(students.id, alumniProfiles.studentId),
        eq(students.institutionId, institutionId)
      )
    )
    .leftJoin(users, eq(users.id, students.userId))
    .where(eq(alumniProfiles.institutionId, institutionId))
    .orderBy(desc(alumniProfiles.updatedAt));
}

export async function upsertAlumniProfile(input: {
  institutionId: number;
  studentId: number;
  ...
  await db
    ...
    .values({ ...input, consentToContact: input.consentToContact ? 1 : 0 })
    .onDuplicateKeyUpdate({
      set: {
        ...
        consentToContact: input.consentToContact ? 1 : 0,
  ...
  const [profile] = await db
    ...
    .where(
      and(
        eq(alumniProfiles.institutionId, input.institutionId),
        eq(alumniProfiles.studentId, input.studentId)
      )
    )
    .limit(1);
  return profile;
...
export async function addAlumniInteraction(input: {
  institutionId: number;
  alumniProfileId: number;
  createdBy: number;
  interactionType: "call" | "email" | "event" | "survey" | "note";
  summary: string;
}) {
  const db = await requireDb();
  const [profile] = await db
    ...
    .where(
      and(
        eq(alumniProfiles.id, input.alumniProfileId),
        eq(alumniProfiles.institutionId, input.institutionId)
      )
    )
    .limit(1);
  ...
  return (
    await db
      ...
      .where(eq(alumniInteractions.id, Number(result[0].insertId)))
      .limit(1)
  )[0];
}

export async function triggerWellbeingNotifications(
  institutionId: number,
  triggerType: "period_end" | "annual_alumni"
) {
  const db = await requireDb();
  const surveys = await db
    ...
    .where(
      and(
        eq(wellbeingSurveys.institutionId, institutionId),
        eq(wellbeingSurveys.triggerType, triggerType),
        eq(wellbeingSurveys.active, 1)
      )
    );
  let created = 0;
  for (const survey of surveys) {
    const recipients =
      survey.audience === "alumni"
        ? await db
            ...
            .where(
              and(
                eq(alumniProfiles.institutionId, institutionId),
                eq(alumniProfiles.consentToContact, 1)
              )
            )
    ...
    for (const recipient of recipients) {
      ...
      await db.insert(notifications).values({
        ...
        title: survey.title,
        message:
          "Tu opinión nos ayuda a mejorar la experiencia institucional. Completa esta encuesta.",
        href,
      });
      created += 1;
    }
...
export async function updateExamAttemptRecording(input: {
  institutionId: number;
  attemptId: number;
  studentId: number;
  recordingKey: string;
