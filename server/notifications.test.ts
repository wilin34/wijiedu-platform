...
function context(role: "admin" | "student" = "student") {
  return {
...
describe("notifications", () => {
  it("solo devuelve las notificaciones del usuario autenticado y permite leerlas", async () => {
    ...
    expect(mocks.markNotificationRead).toHaveBeenCalledWith(7, 1);
    expect(mocks.markAllNotificationsRead).toHaveBeenCalledWith(7);
  });
  ...
  it("permite preferencias personales y reserva la creación a administración", async () => {
    ...
    expect(mocks.createNotification).toHaveBeenCalledWith({ userId: 7, type: "system", title: "Aviso", message: "Mensaje" });
  });
});
